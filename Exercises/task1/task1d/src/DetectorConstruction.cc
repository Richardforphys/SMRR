...
void DetectorConstruction::ConstructField() 
{
  ...
  if(!fieldIsInitialized)
  {
      ...
      // B) Create auxiliary classes, including G4ChordFinder
      //    ************************
      globalFieldMgr->CreateChordFinder(myField);  
      fieldIsInitialized = true;

      // C) Create field for hadronic calorimeter
      //    *************************
      // Create new Field Manager
      G4FieldManager*  pZeroFieldMgr= new G4FieldManager(); 
      //  Set it to 'null' field object -- this means zero field
      G4MagneticField* pNullField= (G4MagneticField*) 0; 
      pZeroFieldMgr->SetDetectorField(pNullField); 

      // D) Override the global field
      //    *************************
